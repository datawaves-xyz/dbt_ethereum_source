{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev1_inverse_basis_point_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV1_INVERSE_BASIS_POINT_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.4.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0xcae6047f"
    and selector_hash = abs(hash("0xcae6047f")) % 10
