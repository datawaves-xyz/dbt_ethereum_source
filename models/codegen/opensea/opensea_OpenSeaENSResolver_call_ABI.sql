{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_openseaensresolver_abi_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_OpenSeaENSResolver_ABI_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.6.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x2203ab56"
    and selector_hash = abs(hash("0x2203ab56")) % 10
