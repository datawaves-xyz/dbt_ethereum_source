{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_openseaensresolver_setauthorisation_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_OpenSeaENSResolver_setAuthorisation_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.3.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x3e9ce794"
    and selector_hash = abs(hash("0x3e9ce794")) % 10
