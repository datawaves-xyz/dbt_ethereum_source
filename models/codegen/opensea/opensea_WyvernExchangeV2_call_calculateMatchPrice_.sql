{{
    config(
        materialized='table',
        file_format='parquet',
        alias='wyvernexchangev2_call_calculatematchprice_',
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev2_calculatematchprice__calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV2_calculateMatchPrice__CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.14.jar";'
        }
    )
}}
...
with base as (
    select
        ...
        dt,
        opensea_wyvernexchangev2_calculatematchprice__calldecodeudf(unhex_input, unhex_output, '{"type": "function", "name": "calculateMatchPrice_", "constant": true, "payable": false, "stateMutability": "view", "inputs": [{"name": "addrs", "type": "address[14]"}, {"name": "uints", "type": "uint256[18]"}, {"name": "feeMethodsSidesKindsHowToCalls", "type": "uint8[8]"}, {"name": "calldataBuy", "type": "bytes"}, {"name": "calldataSell", "type": "bytes"}, {"name": "replacementPatternBuy", "type": "bytes"}, {"name": "replacementPatternSell", "type": "bytes"}, {"name": "staticExtradataBuy", "type": "bytes"}, {"name": "staticExtradataSell", "type": "bytes"}], "outputs": [{"name": "", "type": "uint256"}]}', 'calculateMatchPrice_') as data
    from {{ ref('stg_traces') }}
    where to_address = lower("0x7f268357A8c2552623316e2562D90e642bB538E5") and address_hash = abs(hash(lower("0x7f268357A8c2552623316e2562D90e642bB538E5"))) % 10 and selector = "0xd537e131" and selector_hash = abs(hash("0xd537e131")) % 10
