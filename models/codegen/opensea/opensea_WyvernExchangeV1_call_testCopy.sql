{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev1_testcopy_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV1_testCopy_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.5.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x3e1e292a"
    and selector_hash = abs(hash("0x3e1e292a")) % 10
