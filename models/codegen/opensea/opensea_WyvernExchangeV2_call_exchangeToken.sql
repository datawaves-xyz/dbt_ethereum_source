{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev2_exchangetoken_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV2_exchangeToken_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.5.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0xa25eb5d9"
    and selector_hash = abs(hash("0xa25eb5d9")) % 10
