{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_openseaensresolver_setpubkey_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_OpenSeaENSResolver_setPubkey_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.4.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x29cd62ea"
    and selector_hash = abs(hash("0x29cd62ea")) % 10
