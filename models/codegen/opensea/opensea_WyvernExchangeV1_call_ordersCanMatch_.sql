{{
    config(
        materialized='table',
        file_format='parquet',
        alias='wyvernexchangev1_call_orderscanmatch_',
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev1_orderscanmatch__calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV1_ordersCanMatch__CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.15.jar";'
        }
    )
}}
...
with base as (
    select
        ...
        dt,
        opensea_wyvernexchangev1_orderscanmatch__calldecodeudf(unhex_input, unhex_output, '{"type": "function", "name": "ordersCanMatch_", "constant": true, "payable": false, "stateMutability": "view", "inputs": [{"name": "addrs", "type": "address[14]"}, {"name": "uints", "type": "uint256[18]"}, {"name": "feeMethodsSidesKindsHowToCalls", "type": "uint8[8]"}, {"name": "calldataBuy", "type": "bytes"}, {"name": "calldataSell", "type": "bytes"}, {"name": "replacementPatternBuy", "type": "bytes"}, {"name": "replacementPatternSell", "type": "bytes"}, {"name": "staticExtradataBuy", "type": "bytes"}, {"name": "staticExtradataSell", "type": "bytes"}], "outputs": [{"name": "", "type": "bool"}]}', 'ordersCanMatch_') as data
    from {{ ref('stg_traces') }}
    where to_address = lower("0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b") and address_hash = abs(hash(lower("0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b"))) % 10 and selector = "0x72593b4c" and selector_hash = abs(hash("0x72593b4c")) % 10
