{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev1_validateorder__calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV1_validateOrder__CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.11.jar";'
        }
    )
}}
...
with base as (
    ...
    from {{ ref('stg_traces') }}
    where to_address = lower("0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b") and address_hash = abs(hash(lower("0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b"))) % 10 and selector = "0x60bef33a" and selector_hash = abs(hash("0x60bef33a")) % 10
