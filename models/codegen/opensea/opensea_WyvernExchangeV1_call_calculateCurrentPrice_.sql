{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_wyvernexchangev1_calculatecurrentprice__calldecodeudf as "io.iftech.sparkudf.hive.Opensea_WyvernExchangeV1_calculateCurrentPrice__CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.4.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x3f67ee0d"
    and selector_hash = abs(hash("0x3f67ee0d")) % 10
