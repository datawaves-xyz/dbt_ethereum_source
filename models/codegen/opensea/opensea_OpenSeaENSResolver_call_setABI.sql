{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_openseaensresolver_setabi_calldecodeudf as "io.iftech.sparkudf.hive.Opensea_OpenSeaENSResolver_setABI_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.5.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x623195b0"
    and selector_hash = abs(hash("0x623195b0")) % 10
