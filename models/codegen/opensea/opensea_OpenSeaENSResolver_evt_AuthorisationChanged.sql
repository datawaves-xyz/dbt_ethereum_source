{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function opensea_openseaensresolver_authorisationchanged_eventdecodeudf as "io.iftech.sparkudf.hive.Opensea_OpenSeaENSResolver_AuthorisationChanged_EventDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.14.jar";'
        }
    )
}}
...
final as (
    select
        ...
        contract_address,
        dt,
        data.input.node as node, data.input.owner as owner, data.input.target as target, data.input.isauthorised as isAuthorised
    from base
)
