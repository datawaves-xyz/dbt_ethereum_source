{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function curve_busdswap_calc_token_amount_calldecodeudf as "io.iftech.sparkudf.hive.Curve_BUSDSwap_calc_token_amount_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.6.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0xcf701ff7"
    and selector_hash = abs(hash("0xcf701ff7")) % 10
