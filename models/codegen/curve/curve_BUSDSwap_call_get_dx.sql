{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function curve_busdswap_get_dx_calldecodeudf as "io.iftech.sparkudf.hive.Curve_BUSDSwap_get_dx_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.4.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x67df02ca"
    and selector_hash = abs(hash("0x67df02ca")) % 10
