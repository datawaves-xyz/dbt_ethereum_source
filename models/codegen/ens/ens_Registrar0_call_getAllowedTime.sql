{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function ens_registrar0_getallowedtime_calldecodeudf as "io.iftech.sparkudf.hive.Ens_Registrar0_getAllowedTime_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.15.jar";'
        }
    )
}}
...
final as (
    select
        ...
        contract_address,
        dt,
        data.input._hash as _hash, data.output.output_timestamp as output_timestamp
    from base
)
