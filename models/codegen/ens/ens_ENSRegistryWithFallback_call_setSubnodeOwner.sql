{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function ens_ensregistrywithfallback_setsubnodeowner_calldecodeudf as "io.iftech.sparkudf.hive.Ens_ENSRegistryWithFallback_setSubnodeOwner_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.15.jar";'
        }
    )
}}
...
final as (
    select
        ...
        contract_address,
        dt,
        data.input.node as node, data.input.label as label, data.input.owner as owner, data.output.output_0 as output_0
    from base
)
