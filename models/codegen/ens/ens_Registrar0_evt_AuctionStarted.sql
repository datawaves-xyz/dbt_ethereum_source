{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function ens_registrar0_auctionstarted_eventdecodeudf as "io.iftech.sparkudf.hive.Ens_Registrar0_AuctionStarted_EventDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.12.jar";'
        }
    )
}}
...
final as (
    select
        ...
        contract_address,
        dt,
        data.input.hash as hash, data.input.registrationdate as registrationDate
    from base
)
