{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function rariable_exchangev1_cancel_calldecodeudf as "io.iftech.sparkudf.hive.Rariable_ExchangeV1_cancel_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.6.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0xca120b1f"
    and selector_hash = abs(hash("0xca120b1f")) % 10
