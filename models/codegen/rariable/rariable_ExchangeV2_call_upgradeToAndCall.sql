{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function rariable_exchangev2_upgradetoandcall_calldecodeudf as "io.iftech.sparkudf.hive.Rariable_ExchangeV2_upgradeToAndCall_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.6.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x4f1ef286"
    and selector_hash = abs(hash("0x4f1ef286")) % 10
