{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function yearn_yearngovernance_rewards_calldecodeudf as "io.iftech.sparkudf.hive.Yearn_YearnGovernance_rewards_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.3.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x0700037d"
    and selector_hash = abs(hash("0x0700037d")) % 10
