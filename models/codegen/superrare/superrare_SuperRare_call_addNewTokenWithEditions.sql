{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function superrare_superrare_addnewtokenwitheditions_calldecodeudf as "io.iftech.sparkudf.hive.Superrare_SuperRare_addNewTokenWithEditions_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.6.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x019871e9"
    and selector_hash = abs(hash("0x019871e9")) % 10
