{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function superrare_superrare_salepriceset_eventdecodeudf as "io.iftech.sparkudf.hive.Superrare_SuperRare_SalePriceSet_EventDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.16.jar";'
        }
    )
}}
...
final as (
    select
        ...
        contract_address,
        dt,
        data.input._tokenid as _tokenId, data.input._price as _price
    from base
)
