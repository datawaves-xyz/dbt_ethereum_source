{{
    config(
        ...
        pre_hook={
            'sql': 'create or replace function foundation_market_upgradeto_calldecodeudf as "io.iftech.sparkudf.hive.Foundation_market_upgradeTo_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.5.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x3659cfe6"
    and selector_hash = abs(hash("0x3659cfe6")) % 10
